...
public class SystemInfoHelper {
    ...
    public static double getDimensionTPS(WorldServer worldServer) {
        double worldTickLength = MathHelper
                .mean(MinecraftServer.getServer().worldTickTimes.get(worldServer.provider.dimensionId)) * 1.0E-6D;
        return Math.min(1000.0 / worldTickLength, 20);
    }

    public static double getWorldTickTime(WorldServer worldServer) {
        return MathHelper.mean(MinecraftServer.getServer().worldTickTimes.get(worldServer.provider.dimensionId))
                * 1.0E-6D;
    }
        ...
        for (WorldServer world : MinecraftServer.getServer().worldServers) {
            ...
            textOutput.add(
                    "[" + world.provider.dimensionId
                            ...
                            + ": "
                            + timeFormatter.format(getWorldTickTime(world))
                            + "ms ["
                            + timeFormatter.format(getDimensionTPS(world))
                            + "]");
        }
        textOutput.add("Total Chunks loaded: " + chunksLoaded);
        textOutput.add(
                "Overall: " + timeFormatter.format(MathHelper.mean(MinecraftServer.getServer().tickTimeArray) * 1.0E-6D)
                        + "ms ["
