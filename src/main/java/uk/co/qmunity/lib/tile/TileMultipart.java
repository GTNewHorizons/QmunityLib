...
public class TileMultipart extends TileEntity implements ITilePartHolder {
    ...
    public boolean removePart(IPart part) {
        ...
        if (part.getParent() == null || part.getParent() != this) return false;
        ...
        if (!simulated) {
            ...
            if (!getWorld().isRemote) getWorld().notifyBlocksOfNeighborChange(getX(), getY(), getZ(), blockType);
        }
    ...
    public void sendUpdatePacket(IPart part, int channel) {
        ...
        if (getWorld() != null && getParts().contains(part)) PartUpdateManager.sendPartUpdate(this, part, channel);
    }
    ...
    public void onNeighborBlockChange() {
        ...
        for (IPart p : getParts())
            if (p instanceof IPartUpdateListener) ((IPartUpdateListener) p).onNeighborBlockChange();
    ...
    public void onNeighborChange() {
        ...
        for (IPart p : getParts())
            if (p instanceof IPartUpdateListener) ((IPartUpdateListener) p).onNeighborTileChange();
    ...
    private void onUpdate() {
        ...
        if (!getWorldObj().isRemote) {
            if (getParts().size() == 0) getWorldObj().setBlockToAir(xCoord, yCoord, zCoord);
        }
    }

    public boolean isSideSolid(ForgeDirection face) {

        for (IPart p : getParts()) if (p instanceof IPartSolid) if (((IPartSolid) p).isSideSolid(face)) return true;
    ...
    public void updateEntity() {
        ...
        if (firstTick && loaded) {
            for (IPart p : getParts()) {
                if (p instanceof IPartUpdateListener) ((IPartUpdateListener) p).onLoaded();
            }
            firstTick = false;
        }
        for (IPart p : getParts()) if (p instanceof IPartTicking) ((IPartTicking) p).update();

        if (parts.size() > 100) {
            QLog.error(
                    "A Qmunitylib part has " + parts.size()
                            + " parts! It has been removed. Dimension: "
        ...
        if (shouldDieInAFire) getWorld().setBlockToAir(getX(), getY(), getZ());
    }
    ...
    public List<Vec3dCube> getOcclusionBoxes() {
        ...
        for (IPart p : getParts())
            if (p instanceof IPartOccluding) boxes.addAll(((IPartOccluding) p).getOcclusionBoxes());
    ...
    public int getWeakOutput(ForgeDirection direction) {
        ...
        for (ForgeDirection face : ForgeDirection.VALID_DIRECTIONS) max = Math.max(max, getWeakOutput(direction, face));
    ...
    public boolean canConnect(ForgeDirection direction, ForgeDirection face) {
        ...
        for (IPart p : getParts()) {
            if (p instanceof IPartRedstone) {
                if (p instanceof IPartFace) {
                    if (((IPartFace) p).getFace() == face)
                        if (((IPartRedstone) p).canConnectRedstone(direction)) return true;
                } else {
                    if (((IPartRedstone) p).canConnectRedstone(direction)) return true;
                }
            }
    ...
    public boolean canConnect(ForgeDirection direction) {
        ...
        for (ForgeDirection face : ForgeDirection.VALID_DIRECTIONS) if (canConnect(direction, face)) return true;
    ...
    public void onChunkUnload() {
        ...
        for (IPart p : getParts()) if (p instanceof IPartUpdateListener) ((IPartUpdateListener) p).onUnloaded();
    }
    ...
    public List<IMicroblock> getMicroblocks() {
        ...
        for (IPart p : getParts()) if (p instanceof IMicroblock) microblocks.add((IMicroblock) p);
