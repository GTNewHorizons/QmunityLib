...
/**
 * Base tile entity which provides you a few options. Notably, if you mark a field with @DescSynced, it will be
 * automatically synchronize with the client if the field its value gets changed. If this causes too much traffic, you
 * can additionally mark it with @LazySynced. This will not cause synchronization to occur when this particular field
 * changes, but it will get send along when other fields marked with @DescSynced change, or when sendUpdatePacket() is
 * called manually. Marking @GuiSynced can be used to synchronize fields only with players that have the container open
 * that is associated with this tile entity. Be sure to extend your Container to ContainerBase!
 * 
 * @author MineMaarten
 */
public class TileBase extends TileEntity implements IRotatable, IWorldLocation {
    ...
    /**
     * Before being able to use this, remember to mark the block as redstone emitter by calling
     * BlockContainerBase#emitsRedstone()
     * 
     * @param newValue
     */
    ...
    /**
     * Gets called when the TileEntity ticks for the first time, the world is accessible and updateEntity() has not been
     * ran yet
     */
