...
public abstract class SyncedField<T> {
    ...
    public boolean update() {
        try {
            T value = arrayIndex >= 0 ? getValueForArray(retrieveValue(field, te), arrayIndex)
                    : retrieveValue(field, te);
            ...
            QLog.error(
                    "A problem occured when trying to sync the field of " + te.toString()
                            + ". Field: "
                            + field.toString());
            e.printStackTrace();
        }
