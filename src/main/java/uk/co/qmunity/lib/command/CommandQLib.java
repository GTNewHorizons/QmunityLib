    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (args.length != 2) {
                ...
                String UUID = MinecraftServer.getServer().func_152358_ax().func_152655_a(args[1].toLowerCase()) != null
                        ? MinecraftServer.getServer().func_152358_ax().func_152655_a(args[1].toLowerCase()).getId()
                                .toString()
                        : "Player not found";
            ...
            sender.addChatMessage(
                    new ChatComponentText(
                            "Memory Usage: " + SystemInfoHelper.getAllocatedMem()
                                    + "/"
                                    + SystemInfoHelper.getMaxMem()
                                    + "["
                                    + SystemInfoHelper.getPercentMemUse()
                                    + "%]"));
            ...
            if (sender.getCommandSenderName().equals("Server") || PlayerHelper.isOpped(sender.getCommandSenderName())) {
                if (args.length == 2) {
                    if (args[1].equalsIgnoreCase("showqueue")) {
                        ...
                        sender.addChatMessage(
                                new ChatComponentText(
                                        args[1] + " not online, added to the queue for processing when next online."));
                    }
