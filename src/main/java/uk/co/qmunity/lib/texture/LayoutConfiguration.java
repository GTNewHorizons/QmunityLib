...
public class LayoutConfiguration {
    ...
    protected LayoutConfiguration(BufferedImage img) throws Exception {
        ...
        if (img == null) throw new Exception("You need a valid image in order to create a layout configuration.");
        if (img.getWidth() != img.getHeight()) throw new Exception(
                "The layout isn't symmetrical and a .layout file isn't provided. Loading an empty layout!");
    ...
    protected LayoutConfiguration(BufferedImage img, String file) throws Exception {
        ...
        try {
            obj = (JsonObject) new JsonParser()
                    .parse(new JsonReader(new InputStreamReader(getClass().getResourceAsStream(file))));
        } catch (Exception ex) {
            if (img.getWidth() != img.getHeight()) throw new Exception(
                    "The layout isn't symmetrical and a .layout file isn't provided. Loading an empty layout!");
            return;
        ...
        if (obj.has("width") || obj.has("height")) {
            if (obj.has("width") != obj.has("height")) throw new Exception(
        ...
        if (obj.has("layouts")) {
            ...
            if (layouts * height > img.getHeight()) throw new Exception(
                    "The height of an image of size <layouts> * <layout_height> (" + (layouts * height)
                            + ") exceeds the layout image height ("
                            + img.getHeight()
                            + "). Loading an empty layout!");
        }
    }
    ...
    public BufferedImage getSubLayout(BufferedImage img, int id) {
        ...
        if (id >= layouts) return null;

        if (img.getWidth() != getWidth() || img.getHeight() % getHeight() != 0) return null;
