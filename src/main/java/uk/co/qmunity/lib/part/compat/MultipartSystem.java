...
public enum MultipartSystem {
    ...
    public static List<MultipartSystem> getAvailableSystems() {
        ...
        for (MultipartSystem system : values()) if (system.isLoaded() && system.getCompat() != null) l.add(system);
    ...
    public static void preInit(FMLPreInitializationEvent event) {
        ...
        for (MultipartSystem s : getAvailableSystems()) s.getCompat().preInit(event);
    }

    public static void init(FMLInitializationEvent event) {

        for (MultipartSystem s : getAvailableSystems()) s.getCompat().init(event);
    }

    public static void postInit(FMLPostInitializationEvent event) {

        for (MultipartSystem s : getAvailableSystems()) s.getCompat().postInit(event);
    }
