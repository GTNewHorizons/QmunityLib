...
public class OcclusionHelper {
    ...
    public static PartNormallyOccluded getMicroblockPart(MicroblockShape shape, int size, ForgeDirection... sides) {
        ...
        if (shape == MicroblockShape.FACE) return new PartNormallyOccluded(getFaceMicroblockBox(size, sides[0]));
    ...
    public static boolean microblockOcclusionTest(Vec3i block, MicroblockShape shape, int size,
            ForgeDirection... sides) {

        return microblockOcclusionTest(block.getWorld(), block, shape, size, sides);
    }

    public static boolean microblockOcclusionTest(World world, Vec3i block, MicroblockShape shape, int size,
            ForgeDirection... sides) {

        List<IPart> parts = new ArrayList<IPart>();
        for (IPart part : MultipartCompatibility.getMicroblocks(world, block)) parts.add(part);

        return occlusionTest(parts, getMicroblockPart(shape, size, sides));
    }

    public static boolean microblockOcclusionTest(ITilePartHolder holder, MicroblockShape shape, int size,
            ForgeDirection... sides) {

        return microblockOcclusionTest(holder, false, shape, size, sides);
    }

    public static boolean microblockOcclusionTest(ITilePartHolder holder, boolean allParts, MicroblockShape shape,
            int size, ForgeDirection... sides) {

        List<IPart> parts = new ArrayList<IPart>();
        if (allParts) parts.addAll(holder.getParts());
        for (IPart part : holder.getMicroblocks()) if (!parts.contains(part)) parts.add(part);
    ...
    public static boolean microblockOcclusionTest(ITilePartHolder holder, IMicroblock microblock) {
        ...
        List<IPart> parts = new ArrayList<IPart>();
        for (IPart part : holder.getMicroblocks()) parts.add(part);
    ...
    public static Vec3dCube getFaceMicroblockBox(int size, ForgeDirection face) {
        ...
        return new Vec3dCube(
                face.offsetX > 0 ? 1 - s : 0,
                face.offsetY > 0 ? 1 - s : 0,
                face.offsetZ > 0 ? 1 - s : 0,
                face.offsetX < 0 ? s : 1,
                face.offsetY < 0 ? s : 1,
                face.offsetZ < 0 ? s : 1);
    }
    ...
    public static Vec3dCube getHollowFaceMicroblockBox(int size, ForgeDirection face) {
        ...
        return new Vec3dCube(
                face.offsetX > 0 ? 1 - s : (face.offsetX < 0 ? 0 : 0.5 - d),
                face.offsetY > 0 ? 1 - s : (face.offsetY < 0 ? 0 : 0.5 - d),
                face.offsetZ > 0 ? 1 - s : (face.offsetZ < 0 ? 0 : 0.5 - d),
                face.offsetX < 0 ? s : (face.offsetX > 0 ? 1 : 0.5 + d),
                face.offsetY < 0 ? s : (face.offsetY > 0 ? 1 : 0.5 + d),
                face.offsetZ < 0 ? s : (face.offsetZ > 0 ? 1 : 0.5 + d));
    }
    ...
    public static Vec3dCube getEdgeMicroblockBox(int size, ForgeDirection side1, ForgeDirection side2) {
        ...
        return new Vec3dCube(
                (side1.offsetX == 0 && side2.offsetX == 0) ? s : (x ? 1 - s : 0),
                (side1.offsetY == 0 && side2.offsetY == 0) ? s : (y ? 1 - s : 0),
                (side1.offsetZ == 0 && side2.offsetZ == 0) ? s : (z ? 1 - s : 0),
                (side1.offsetX == 0 && side2.offsetX == 0) ? 1 - s : (x ? 1 : s),
                (side1.offsetY == 0 && side2.offsetY == 0) ? 1 - s : (y ? 1 : s),
                (side1.offsetZ == 0 && side2.offsetZ == 0) ? 1 - s : (z ? 1 : s));
    }

    public static Vec3dCube getCornerMicroblockBox(int size, ForgeDirection side1, ForgeDirection side2,
            ForgeDirection side3) {
    ...
    public static boolean occlusionTest(World world, Vec3i block, Collection<Vec3dCube> boxes) {
        ...
        if (holder != null) return occlusionTest(holder, boxes);
    ...
    public static boolean occlusionTest(ITilePartHolder holder, Collection<Vec3dCube> boxes) {
        ...
        for (Vec3dCube box : boxes) if (!occlusionTest(holder, box)) return false;
    ...
    public static boolean occlusionTest(Collection<IPart> parts, Collection<Vec3dCube> boxes) {
        ...
        for (IPart p : parts) for (Vec3dCube box : boxes) if (!occlusionTest(p, box)) return false;
    ...
    public static boolean occlusionTest(IPart part, Collection<Vec3dCube> boxes) {
        ...
        for (Vec3dCube box : boxes) if (!occlusionTest(part, box)) return false;
    ...
    public static boolean occlusionTest(World world, Vec3i block, Vec3dCube box) {
        ...
        if (holder != null) return occlusionTest(holder, box);
    ...
    public static boolean occlusionTest(World world, Vec3i block, IPart part) {
        ...
        if (holder != null) return occlusionTest(holder, part);
    ...
    public static boolean occlusionTest(Collection<IPart> parts, IPart part) {
        ...
        for (IPart p : parts) if (!p.occlusionTest(part) || !part.occlusionTest(p)) return false;
    ...
    public static boolean occlusionTest(IPart part, IPart part2) {
        ...
        if (part instanceof IPartOccluding && part2 instanceof IPartOccluding) {
            ...
            for (Vec3dCube c1 : p1.getOcclusionBoxes())
                for (Vec3dCube c2 : p2.getOcclusionBoxes()) if (!c1.occlusionTest(c2)) return false;
        }
